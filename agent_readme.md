...
## 1. System Context
You are modifying a production-grade Google Apps Script environment engineered by @atriganguly. This system performs bulk network requests to the Google PageSpeed Insights API, parses the data, and batch-writes to a Google Spreadsheet. It utilizes a stateful continuation architecture ("Relay Runner") to bypass maximum execution time limits.
...
* **Strict API Abstraction:** Never hardcode API keys, passwords, or sensitive IDs. Always utilize `PropertiesService.getScriptProperties()`.
...
* **Decoupled Configuration:** Do not place structural mappings (column numbers, sheet names, email templates, limits) inside the core logic files. All mappings must reside in the `CONFIG` constant within `ppm-configurator.gs`. Ensure `ORG_ID`, `ORGANIZATION`, and `LABEL` metadata columns are respected during map shifts.
